  ...
  return (
    <Svg height={size} viewBox="0 0 100 100" width={size}>
      <Polygon
        ...
        strokeWidth={4}
      />
      <Circle cx="50" cy="50" fill={strokeColor} opacity={opacity * 0.75} r="5" />
    </Svg>
  );
}
...
export function AndalusianArch({
  ...
  const { colors } = useTheme();
  const strokeColor = color ?? colors.goldSoft;
  return (
    ...
    <Svg height="100%" width="100%">
      <Defs>
        <Pattern
          ...
          width="64"
        >
          <G fill="none" opacity={Math.min(0.05, opacity)} stroke={strokeColor} strokeWidth={1}>
...
export function GeometricDivider({
  ...
  return (
    <View accessibilityElementsHidden importantForAccessibility="no" style={styles.divider}>
      ...
      <IslamicStar color={color ?? colors.gold} opacity={0.8} size={17} />
      <View style={styles.dividerLine} />
    </View>
  );
}
