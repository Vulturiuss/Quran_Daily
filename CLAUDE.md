...
**State.** `src/store/useQuranStore.ts` is the single Zustand store, persisted to AsyncStorage (`quran-daily-state`, currently `version: 6`). It owns onboarding, per-surah progress, stats, session history, the active session, and `syncMeta`. Every mutating action must also set `syncMeta` via `changedNow(...)` — that dirty flag is what triggers a cloud push. Bumping the persisted shape means bumping `version` and extending `migrate`.
